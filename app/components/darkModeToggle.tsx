...
export default function DarkModeToggle(){
  ...
  return(
    <button className="select-none flex items-center w-full aspect-square rounded-full relative bg-zinc-300/15 backdrop-blur-xs shadow-lg overflow-hidden border-2 border-white/40
        ...
        <div className='relative flex items-center justify-center w-full aspect-square overflow-hidden rounded-full'>
          <LightModeIcon sx={{fontSize: {xs: 56,sm: 64, md: 72, lg: 80}, filter: 'drop-shadow(4px 4px 1px rgba(0, 0, 0, 0.2))', transition: 'translate 400ms ease-in-out, opacity 500ms ease-in-out',}} className={`text-amber-400 absolute ${isLight ? 'translate-y-0 opacity-100' : 'translate-y-full opacity-0'}`} />
          <DarkModeIcon sx={{fontSize: {xs: 56,sm: 64, md: 72, lg: 80}, filter: 'drop-shadow(4px 4px 1px rgba(0, 0, 0, 0.2))', transition: 'translate 400ms ease-in-out, opacity 500ms ease-in-out',}} className={`text-indigo-600 absolute ${isLight ? 'translate-y-full opacity-0' : 'translate-y-0 opacity-100'}`} />
        </div>
    </button>
  )
}