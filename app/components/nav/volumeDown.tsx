...
export default function VolumeDown(){
  ...
  return (
    ...
    before:content-['Volume_Down'] before:pointer-events-none before:absolute before:w-max before:bg-linear-to-t before:from-zinc-300/70 before:to-white before:border-2 before:border-white/75 before:shadow-lg before:font-semibold before:p-4 before:rounded-xl before:top-[-50%] before:left-[50%] before:translate-[-50%] before:scale-0 before:opacity-0 before:transition-all before:ease-in
    hover:before:scale-100 hover:before:opacity-100">{
      <div className="max-w-20 aspect-square">
        <VolumeDownIcon sx={{fontSize: 80, filter: 'drop-shadow(4px 4px 1px rgba(0, 0, 0, 0.2))'}} className="text-red-500" />
      </div>
    }</button>
  )
}